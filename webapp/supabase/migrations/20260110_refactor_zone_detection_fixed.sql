-- Migration: Refonte système de détection des zones faciales (CORRIGÉE)
-- Date: 2026-01-10
...
DROP TRIGGER IF EXISTS update_zone_templates_updated_at ON zone_templates;
CREATE TRIGGER update_zone_templates_updated_at
  BEFORE UPDATE ON zone_templates
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_detected_face_zones_updated_at ON detected_face_zones;
...
-- ============================================================================
-- 9. PERMISSIONS (RLS - Row Level Security) - SIMPLIFIÉ
-- ============================================================================
...
-- Politique pour zone_templates (lecture publique pour tous les utilisateurs authentifiés)
DROP POLICY IF EXISTS "Zone templates are viewable by all authenticated users" ON zone_templates;
CREATE POLICY "Zone templates are viewable by all authenticated users"
  ON zone_templates FOR SELECT
  TO authenticated
  USING (true);

DROP POLICY IF EXISTS "Zone templates are insertable by all authenticated users" ON zone_templates;
CREATE POLICY "Zone templates are insertable by all authenticated users"
  ON zone_templates FOR INSERT
  TO authenticated
  WITH CHECK (true);

DROP POLICY IF EXISTS "Zone templates are updatable by all authenticated users" ON zone_templates;
CREATE POLICY "Zone templates are updatable by all authenticated users"
  ON zone_templates FOR UPDATE
  TO authenticated
  USING (true)
  WITH CHECK (true);

DROP POLICY IF EXISTS "Zone templates are deletable by all authenticated users" ON zone_templates;
CREATE POLICY "Zone templates are deletable by all authenticated users"
  ON zone_templates FOR DELETE
  TO authenticated
  USING (true);

-- Politique pour detected_face_zones (accès complet pour tous les utilisateurs authentifiés)
DROP POLICY IF EXISTS "Detected zones are accessible by all authenticated users" ON detected_face_zones;
CREATE POLICY "Detected zones are accessible by all authenticated users"
  ON detected_face_zones FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);

-- Politique pour photo_quality_metrics (accès complet pour tous les utilisateurs authentifiés)
DROP POLICY IF EXISTS "Photo quality metrics are accessible by all authenticated users" ON photo_quality_metrics;
CREATE POLICY "Photo quality metrics are accessible by all authenticated users"
  ON photo_quality_metrics FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 20260110_refactor_zone_detection (FIXED) completed successfully';
  ...
  RAISE NOTICE 'RLS policies simplified for all authenticated users';
